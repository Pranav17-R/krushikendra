  ...
  async function loadProducts(containerId, options = {}) {
    ...
    if (!result.ok) {
      container.innerHTML = `<p style="color:#991b1b;text-align:center;">
        ⚠️ Could not load products. ${result.message || ''}
      </p>`;
      return;
    }
    ...
    container.innerHTML = products.map(p => `
      <div class="product-card fade-in">
        <div class="product-img-wrap" style="${p.image ? 'padding: 0; position: relative; overflow: hidden;' : ''}">
          ${p.image ? `<img src="${p.image}" alt="${escHtml(p.name)}" class="product-img">` : `<div class="product-emoji">${p.emoji || '🌿'}</div>`}
          <span class="product-cat-badge" style="${p.image ? 'position: absolute; top: 10px; right: 10px;' : ''}">${p.categoryLabel || p.category}</span>
          <span class="stock-badge ${stockClass(p.stock)}" style="${p.image ? 'position: absolute; top: 10px; left: 10px;' : ''}">${p.stock || 'In Stock'}</span>
        </div>
        <div class="product-body">
          <h3 class="product-name">${escHtml(p.name)}</h3>
          <p class="product-desc">${p.description.length > 100 ? escHtml(p.description.substring(0, 100)) + '...' : escHtml(p.description)}</p>
